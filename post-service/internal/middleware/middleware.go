...
func Auth(cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token, err := jwt.ParseWithClaims(strings.TrimPrefix(header, "Bearer "), &jwtClaims{},
...
func Logger() gin.HandlerFunc {
	return func(c *gin.Context) {
		start := time.Now()
		c.Next()
		log.Printf("[post] %s %s %d %s", c.Request.Method, c.Request.URL.Path, c.Writer.Status(), time.Since(start))
	}
}
